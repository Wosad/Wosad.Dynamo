...
/// 
    ...
    {
/// <summary>
///     Connection interface forces per Uniform Force Method for brace connection.  Case when gusset is connected to beam only.
/// </summary>
    ...
    /// <returns name="M_ubc"> Moment at beam-column interface </returns>
    /// <returns name="V_ubc"> Required shear force on the beam-column connection </returns>

        [MultiReturn(new[] { "H_ub","V_ub","M_ub","M_ubc","V_ubc" })]
        public static Dictionary<string, object> UFMForcesSpecialCase3(double d_b,double d_c,double theta,
            double alpha,double alpha_bar,double P_u,double R_beam,
            bool IncludeDistortionalMomentForces=false,double M_d=0,double A_ub=0)
        {
            //Default values
            double H_ub = 0;
            double V_ub = 0;
            ...
            UFMCase3NoGussetToColumnConnection ufmCase = new UFMCase3NoGussetToColumnConnection(
                d_b, d_c, theta, alpha, alpha_bar, P_u, R_beam, IncludeDistortionalMomentForces, M_d, A_ub);
